...
function buildAnalysisPrompt(selectedText, context, provider = "gemini") {
  ...
  if (words.length > 2) {
    return `Context: "${context}".  Selected: "${selectedText}". Paraphrase ONLY the selected text using different words. Do NOT add any details from the context that aren't in the selected.`;
  }
